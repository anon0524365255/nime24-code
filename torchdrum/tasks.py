...
from torchdrum.feature import CascadingFrameExtactor
from torchdrum.feature import FeatureCollection
from torchdrum.synth import AbstractSynth
...
class TorchDrumParameterMapping(L.LightningModule):
    """
    A LightningModule to train TorchDrum parameter mappings
    """
...
class TorchDrumIntervalMatch(L.LightningModule):
    """
    A LightningModule to use gradient descent to find synthesis parameters to
    match a target timbre interval
    """
    ...
    def __init__(
        ...
        # Create a parameter modulation parameter
        self.param_mod = torch.nn.Parameter(torch.zeros_like(self.preset))
    ...
    def _do_step(self, batch, batch_idx):
        ...
        # Calculate features from the synthesized audio
        y = self()
        y_features = self.feature(y)
