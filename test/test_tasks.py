...
from torchdrum.loss import FeatureDifferenceLoss
from torchdrum.model import MLP
from torchdrum.synth import SimpleDrumSynth
...
def test_init_torchdrum_parameter_mapping(features, preset_json):
    synth = SimpleDrumSynth(sample_rate=44100, num_samples=44100)
    model = MLP(in_size=1, hidden_size=1, out_size=1, num_layers=1)
    _ = TorchDrumParameterMapping(
        model=model,
        synth=synth,
        preset=preset_json,
        feature=features,
...
def test_torchdrum_parameter_mapping_forward(features, preset_json):
    synth = SimpleDrumSynth(sample_rate=44100, num_samples=44100)
    num_params = synth.get_num_params()
    model = MLP(in_size=1, hidden_size=1, out_size=num_params, num_layers=1)
    mapping = TorchDrumParameterMapping(
        model=model,
        synth=synth,
        preset=preset_json,
        feature=features,
...
def test_torchdrum_parameter_mapping_train_step(features, preset_json):
